...
class TensorRTInfer:
    ...
    def infer(self, batch, top=1):
        ...
        output = np.zeros(*self.output_spec())  # 分配结果的内存空间

        # Process I/O and execute the network
        cuda.memcpy_htod(self.inputs[0]["allocation"], np.ascontiguousarray(batch)) # 将内存中的图片移动到显存上
        self.context.execute_v2(self.allocations)
        cuda.memcpy_dtoh(output, self.outputs[0]["allocation"])                     # 将显存中的结果移动到内存上

        # Process the results
        classes = np.argmax(output, axis=1)
        scores = np.max(output, axis=1)
        top = min(top, output.shape[1])
        top_classes = np.flip(np.argsort(output, axis=1), axis=1)[:, 0:top] # np.flip() 翻转顺序
...
def main(args):
    ...
    for batch, images in batcher.get_batch():
        classes, scores, top = trt_infer.infer(batch, args.top)
...
if __name__ == "__main__":
    ...
    parser.add_argument("-e", "--engine", default="efficientnet_b0.engine", help="The TensorRT engine to infer with")
    parser.add_argument(
        "-i", "--input", default="dog.jpg", help="The input to infer, either a single image path, or a directory of images"
    )
    parser.add_argument(
